...
<?php
    if (isset($_POST['login'])){
        ...
        $result = mysqli_query($conn, "SELECT * FROM nhanvien where email = '$email'");
        $row = mysqli_fetch_array($result);
        if (password_verify($pass, $row['matkhau'])){
            setcookie( 'type' , 'admin' , time () + (604800) , "/");
            setcookie( 'login' , $row['idnv'] , time () + (604800) , "/");
            ?>
                <script>window.location.href = 'master.php';</script>
            <?php
        }else{
...
<body>
    ...
    <form method="POST">
        ...
        <input type="text" name="email" placeholder="Email" id="username"
            value="<?php if (isset($_POST['login'])) echo $_POST['email'];?>"
            required
        >
